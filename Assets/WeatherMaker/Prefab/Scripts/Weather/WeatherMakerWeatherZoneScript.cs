...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerWeatherZoneScript : MonoBehaviour
    {
        ...
        [SingleLine("Override random duration for profiles to transition in (set to 0 to not override)")]
        public RangeOfFloats TransitionDuration = new RangeOfFloats { Minimum = 0, Maximum = 0 };

        [SingleLine("Override random duration for profiles to hold before transition to another profile (set to 0 to not override)")]
        ...
        private void UpdateProfile()
        {
            ...
            else if (SingleProfile == null)
            {
                ...
                if (secondsRemainingTransition <= 0.0f)
                {
                    if ((secondsRemainingHold -= Time.deltaTime) <= 0.0f)
                    {
                        ...
                        currentProfile = WeatherMakerProfileGroupScript.OverrideProfile(currentProfile, CloudProfile, PrecipitationProfile, FogProfile, WindProfile, LightningProfile, SoundProfile, TransitionDuration, HoldDuration);
            ...
            else if (SingleProfile != currentProfileSingle)
            {
                ...
                WeatherMakerProfileScript oldProfile = currentProfile;
                currentProfile = WeatherMakerProfileGroupScript.OverrideProfile(SingleProfile, CloudProfile, PrecipitationProfile, FogProfile, WindProfile, LightningProfile, SoundProfile, TransitionDuration, HoldDuration);
                secondsRemainingTransition = currentProfile.RandomTransitionDuration();
