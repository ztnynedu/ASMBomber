...
namespace DigitalRuby.WeatherMaker
{
    public abstract class WeatherMakerThunderAndLightningScript : MonoBehaviour
    {
        ...
        private void Strike(Vector3? _start, Vector3? _end, bool intense, float intensity, Camera camera, bool forceVisible)
        {
            ...
            if (WeatherMakerLightManagerScript.Instance == null || WeatherMakerLightManagerScript.Instance.Sun == null)
            {
                intensityModifier = 0.35f;
            }
            else if (WeatherMakerScript.ResolveCameraMode() != CameraMode.Perspective)
            {
        ...
        public static WeatherMakerThunderAndLightningScript Instance
        {
            get
            {
                if (instance == null)
                {
                    foreach (WeatherMakerThunderAndLightningScript script in GameObject.FindObjectsOfType<WeatherMakerThunderAndLightningScript>())
                    {
                        if (script.enabled)
                        {
                            instance = script;
