...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerCloudShadowProjectorScript : MonoBehaviour
    {
        ...
        private Texture2D cloudShadowTexturePixels;
        ...
        private Vector3 GetSunCameraIntersect(Camera c)
        {
            ...
            ray.y += WeatherMakerScript.Instance.CloudScript.CloudProfile.CloudLayer1.CloudRayOffset;
            ...
            Vector3 plane = new Vector3(cameraPos.x, WeatherMakerScript.Instance.CloudScript.CloudProfile.CloudLayer1.CloudHeight, cameraPos.z);
            ...
            CloudShadowMaterial.SetVector("_CloudShadowCenterPoint", intersect);

            // reduce shadow when sun is low in the sky
            CloudShadowMaterial.SetFloat("_CloudShadowMultiplier", multiplier);
        ...
        private Vector3? ProcessCloudShadowsAndGetSunCameraIntersect(Camera c)
        {
            ...
            (
                WeatherMakerScript.Instance.CloudScript.CloudProfile.CloudHasShadows &&
        ...
        private void Start()
        {
            cloudShadowTexturePixels = new Texture2D(1, 1, TextureFormat.ARGB32, false, false);

#if UNITY_EDITOR

            if (Application.isPlaying)
            {
                CloudShadowMaterial = new Material(CloudShadowMaterial);
            }
        ...
        {
            Color32 sunPixel = cloudShadowTexturePixels.GetPixel(0, 0);
            if (sunPixel.a > 70)
            {
                Debug.Log("Sun Blocked!");
            }
        }
        ...
        public void RenderCloudShadows(Camera c)
        {
            ...
            {
                float scale = c.farClipPlane * WeatherMakerScript.Instance.SkySphereScript.SkySphereProfile.FarClipPlaneMultiplier;
            ...
            Graphics.CopyTexture(cloudShadowTexture, 0, 0, cloudShadowTexture.width / 2, cloudShadowTexture.height / 2, cloudShadowTexturePixels.width, cloudShadowTexturePixels.height, cloudShadowTexturePixels, 0, 0, 0, 0);
            //cloudShadowTexturePixels.ReadPixels(new Rect(cloudShadowTexture.width / 2, cloudShadowTexture.height / 2, cloudShadowTexturePixels.width, cloudShadowTexturePixels.height), 0, 0);
            Shader.SetGlobalTexture("_WeatherMakerDebugTexture", cloudShadowTexture);
        }
        */
    }
}
