...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerFullScreenCloudsScript : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            {
                WeatherMakerLightManagerScript.Instance.UpdateShaderVariables(effect.Material, cloudCollider);
            }
        }
        ...
        public void ShowCloudsAnimated(WeatherMakerCloudProfileScript newProfile, float transitionDelay, float transitionDuration, string tweenKey = null)
        {
            ...
            FloatTween tween = TweenFactory.Tween("WeatherMakerClouds_" + GetInstanceID() + tweenKey, 0.0f, 1.0f, transitionDuration, TweenScaleFunctions.Linear, (ITween<float> c) =>
            {
        ...
        public static WeatherMakerFullScreenCloudsScript Instance
        {
            get
            {
                if (instance == null)
                {
                    foreach (WeatherMakerFullScreenCloudsScript script in GameObject.FindObjectsOfType<WeatherMakerFullScreenCloudsScript>())
                    {
                        if (script.enabled)
                        {
                            instance = script;
