...
uniform fixed4 _WeatherMakerSunColorSky; // sun color in the sky, cloud height-ish
...
fixed3 GetNightColor(float3 ray, float2 uv, float light)
{
	...
	{
		fixed moonDot = pow(max(0.0, dot(_WeatherMakerMoonDirectionUp[i], ray)), _WeatherMakerMoonLightPower[i].x);
		nightColor += (moonDot * _WeatherMakerMoonLightColor[i].rgb * _WeatherMakerMoonLightColor[i].a * _WeatherMakerMoonLightPower[i].y);
	}

	nightColor += _NightTexAddColor;
	return nightColor * _NightTexTintColor * _NightIntensity * _NightSkyMultiplier * _WeatherMakerNightMultiplier * (1.0 - light);
}
