...
float ComputeCloudFBMOutter(float3 rayDir, float3 worldPos, sampler2D noiseTex, sampler2D maskTex)
{
...
fixed3 ComputeDirectionalLightCloud(fixed4 lightColor, float3 lightDir, float3 rayDir, float fbm, float scatterMultiplier, float powerX, float powerY)
{
	...
	float lightMultiplier = (lightDot * max(lightColor.a, 0.2) * scatterMultiplier);

	// indirect light
	float indirectLight = (lightColor.a * lightColor.a);
	indirectLight /= pow(fbm, 0.5);

	return ((lightColor.rgb * indirectLight) + (lightColor.rgb * lightMultiplier * lightMultiplier * powerY));
}
...
fixed3 ComputeCloudLighting(fixed3 cloudColor, float fbm, float3 rayDir, float3 worldPos, fixed alphaAccum)
{
	...
	fixed3 cloudDirLight = ComputeDirectionalLightCloud(_WeatherMakerSunColorSky, _WeatherMakerSunDirectionUp, rayDir, fbm, scatterMultiplier, _WeatherMakerSunLightPower.x, _WeatherMakerSunLightPower.y);

	// moonlight
	UNITY_LOOP
	for (int i = 0; i < _WeatherMakerMoonCount; i++)
	{
		cloudDirLight += ComputeDirectionalLightCloud(_WeatherMakerMoonLightColor[i], _WeatherMakerMoonDirectionUp[i], rayDir, fbm, scatterMultiplier, _WeatherMakerMoonLightPower[i].x, _WeatherMakerMoonLightPower[i].y);
	}
	...
	p.ambientColor = _WeatherMakerAmbientLightColorSky.rgb * _CloudAmbientMultiplier[_CloudIndex];
	p.shadowStrength = 0.0;
	cloudColor = fbm * CalculateLightColorWorldSpace(p).rgb;
	cloudColor += (cloudDirLight * dirLightDensityFactor);

	return cloudColor;
}

fixed4 ComputeCloudColor(float3 rayDir, sampler2D noiseTex, sampler2D maskTex, float2 screenUV, inout fixed alphaAccum)
{
	float3 worldPos = CloudRaycastWorldPos(rayDir);
