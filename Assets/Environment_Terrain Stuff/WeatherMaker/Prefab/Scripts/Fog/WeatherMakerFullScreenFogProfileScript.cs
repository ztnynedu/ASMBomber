...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerFullScreenFogProfileScript : WeatherMakerFogProfileScript
    {
        ...
        public override void UpdateMaterialProperties(Material material, Camera camera)
        {
            ...
            material.SetFloat("_FogHeight", Mathf.Max(0.0f, FogHeight));
            if ((WeatherMakerScript.Instance == null || WeatherMakerScript.Instance.EnableFogFullScreenSunShafts) &&
                (SunShaftSampleCount > 0 && WeatherMakerLightManagerScript.Instance.Sun.LightIsOn && WeatherMakerLightManagerScript.Instance.Sun.ViewportPosition.z > 0.0f &&
                SunShaftTintColor.a > 0.0f))
            {
                // sun shafts are visible
                material.SetInt("_FogSunShaftMode", 1);
                bool gamma = (QualitySettings.activeColorSpace == ColorSpace.Gamma);
                float brightness = (WeatherMakerFullScreenCloudsScript.Instance == null ? 1.0f : SunShaftBrightness * (gamma ? 1.0f : 0.33f) * Mathf.Max(0.0f, 1.0f - (WeatherMakerFullScreenCloudsScript.Instance.CloudProfile.CloudCoverTotal * 1.5f)));
                ...
                material.SetVector("_FogSunShaftsDitherMagic", new Vector4(SunShaftDitherMagic.x * Screen.width , SunShaftDitherMagic.y * Screen.height, SunShaftDitherMagic.z * Screen.width, SunShaftDitherMagic.w * Screen.height));
            }
            else
            {
                // sun shafts not visible
                material.SetInt("_FogSunShaftMode", 0);
            }
        }
    }
}
