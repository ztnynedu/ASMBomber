...
namespace DigitalRuby.WeatherMaker
{
    ...
    public enum WeatherMakerCameraType
    {
        ...
        /// <summary>
        /// Pre-render camera, internal Unity use, should generally be ignored
        /// </summary>
        PreRender
    }
    ...
    public class WeatherMakerCommandBufferManagerScript : MonoBehaviour
    {
        ...
        /// <summary>
        /// Set to any camera you are calling RenderToCubemap with
        /// </summary>
        ...
        //private bool needsRecreateCommandBuffers;
        ...
        private void UpdateCommandBuffersForCamera(Camera camera)
        {
            ...
            foreach (WeatherMakerCommandBuffer commandBuffer in commandBuffers)
            {
                ...
                if (commandBuffer.CameraType == WeatherMakerCameraType.Reflection)
                {
                    // find the camera observing this reflection camera
                    WeatherMakerReflectionScript refl = (camera.transform.parent == null ? camera.transform.GetComponent<WeatherMakerReflectionScript>() : camera.transform.parent.GetComponent<WeatherMakerReflectionScript>());
                    if (refl == null)
                    {
                    ...
                    {
                        camera = refl.CameraRenderingReflection(camera);
                        if (camera == null)
                        {
                            camera = lastNonReflectionCamera == null ? camera : lastNonReflectionCamera;
                        }
                ...
                else if (commandBuffer.CameraType == WeatherMakerCameraType.CubeMap)
                {
                    // ???
                }
                else
                ...
                if (camera.stereoEnabled)
                {
                    ...
                    inverseProj[0] = left_screen_from_view;
                    inverseProj[1] = right_view_from_screen;
                }
                else
                {
                    ...
                    inverseProj[0] = inverseProj[1] = left_view_from_screen;
                }
                commandBuffer.Material.SetMatrixArray("_WeatherMakerInverseView", inverseView);
                commandBuffer.Material.SetMatrixArray("_WeatherMakerInverseProj", inverseProj);
                if (commandBuffer.CameraType == WeatherMakerCameraType.CubeMap || commandBuffer.Camera == CubemapCamera)
                {
                    commandBuffer.Material.SetInt("_WeatherMakerCameraRenderMode", 1);
                }
                else if (commandBuffer.CameraType == WeatherMakerCameraType.Reflection)
                {
                    commandBuffer.Material.SetInt("_WeatherMakerCameraRenderMode", 2);
                }
                else
                {
                    commandBuffer.Material.SetInt("_WeatherMakerCameraRenderMode", 0);
                }
        ...
        private void LateUpdate()
        {
            ...
            /*
            // if you see stuff turn black, just move the mouse back over the Unity editor and all will be well
            // alternatively, you can deactivate and reactivate the weather maker prefab
            if (UnityEditor.EditorWindow.mouseOverWindow == null)
            {
                needsRecreateCommandBuffers = true;
            }
            else if (needsRecreateCommandBuffers)
            {
                needsRecreateCommandBuffers = false;
