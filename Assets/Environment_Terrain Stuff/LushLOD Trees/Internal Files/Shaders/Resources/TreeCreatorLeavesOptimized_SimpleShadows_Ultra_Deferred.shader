...
Shader "Hidden/LushLODTree/Tree Creator Leaves Optimized Simple Shadows Ultra Deferred" {
	...
	[HideInInspector] _TreeInstanceColor("TreeInstanceColor", Vector) = (1,1,1,1)
		...
		CGPROGRAM
#pragma surface surf LushLODTree vertex:LushLODTreeVert nolightmap noforwardadd noambient addshadow keepalpha //<-- removed "alphatest:_Cutoff" for LushLOD trees, since we calculate the cutoff below. I haven't tested "noshadow" yet.
#pragma target 3.0
...
//			float2 uv : TEXCOORD1;
//		float4 pos : TEXCOORD3; //<- Added for LushLOD trees.
//		float4 screenPos : TEXCOORD4; //<- Added for LushLOD trees.
//	};
//	float4 _MainTex_ST;
...
//
//		//o.pos = UnityObjectToClipPos(v.vertex); //<- Added for LushLOD trees.
...
//	////////////////////	//i.screenPos.y *= _ProjectionParams.x; //<-- TODO: This doesn't match the surf function.
//	////////////////////
//	////////////////////	screenPos.y *= _ProjectionParams.x;
//	////////////////////	screenPos.y = fmod(_ScreenParams.y, 2) == 1 ? -screenPos.y : screenPos.y;
//	////////////////////
//	////////////////////	float2 sceneUVs = screenPos.xy;// float2(1, grabSign)*i.screenPos.xy*0.5 + 0.5;
//	////////////////////	float Tex_Alpha = alpha;
//	////////////////////	float Param_Cutoff = _Cutoff;
//	////////////////////	float Param_Transparency = _Transparency;
//	////////////////////	float Alpha_Smoothing = saturate(min(Param_Transparency, Tex_Alpha));
//	////////////////////	float Dither = Dither_SMOOTHING(Tex_Alpha, XYtest(sceneUVs.rg), Param_Cutoff, Alpha_Smoothing); // Send to post processing shader.
//	////////////////////																									//clip(Dither - 0.5);
//	////////////////////
//	////////////////////
//	////////////////////	//i.screenPos = float4(i.screenPos.xy / i.screenPos.w, 0, 0);
...
//
//
//	float cut_off = _Cutoff;
//	float2 sceneUVs = float2(1, grabSign)*i.screenPos*0.5 + 0.5;
//	float matrixIN = XYtest(sceneUVs.rg);
//
//	cut_off += (1 - _LushLODTreeDisableHQ); //<-- clips the whole HQ tree if the _LushLODTreeDisableHQ global variable is set to zero.
//	clip(alpha - cut_off); //<-- EDIT: Instead of the line above, we can just clip it now (looks slightly better?)
//						   //////Clipped_alpha = 1; //<-- we clipped it, so everything left is solid 1 alpha.
