...
Shader "Hidden/LushLODTree/Tree Creator Leaves Optimized Simple Shadows" {
	...
	[HideInInspector] _TreeInstanceColor("TreeInstanceColor", Vector) = (1,1,1,1)
		...
		CGPROGRAM
		#pragma surface surf LushLODTree vertex:TreeVertLeaf nolightmap noforwardadd noambient noshadow //keepalpha //<-- removed "alphatest:_Cutoff" for LushLOD trees, since we calculate the cutoff below. I haven't tested "noshadow" yet.
		#pragma target 3.0
	...
	//float3 CheckLightmap2(float3 lightCol, float3 dummyREF, float dummyREF2, float w) {  //<- Added for LushLOD trees.
	//	return w == 2 ? _LushLODTreeMainLightCol.rgb
	//		* _LushLODTreeMainLightIntensity / 1.2
	//		: lightCol;
	//}
	...
	inline half4 LightingLushLODTree(LeafSurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
	{
		...
		_LightColor0.rgb = _LushLODTreeMainLightCol.rgb;// CheckLightmap2(_LightColor0.rgb, 0, 0, _LushLODTreeMainLightDir.a);// + _LushLODTreeAmbientColor.rgb);
	...
	void surf(Input i, inout LeafSurfaceOutput o) {
		...
		float2 sceneUVs = float2(1, grabSign)*i.uv_MainTex*0.005 + 0.005;  //float2 sceneUVs = float2(1, grabSign)*i.screenPos.xy*0.5 + 0.5;
		...
		//float2 screenUVs = float2(1, grabSign)*i.screenPos.xy * 1 + 1;
		...
		o.Albedo = finalColor * saturate(((i.color.a * _LushLODTreeAO) - (_LushLODTreeAO - 1)) + saturate(1.0 - _Transparency)); //<-- Modified for LushLOD Trees
				...
				//float3 LightIN = node_3973;// (CheckLightmap(_LightColor0.rgb, _LushLODTreeMainLightDir.a) + _LushLODTreeAmbientColor.rgb);
...
#pragma vertex vert_surf
		...
		#include "UnityBuiltin3xTreeLibrary.cginc"
			...
			fixed _Cutoff;
			float4 frag_surf(v2f_surf i) : SV_Target{
				half alpha = tex2D(_MainTex, i.texPos).a;
			...
			//////////*************************************
				...
				i.texPos.y *= _ProjectionParams.x;

				float2 sceneUVs = float2(1, grabSign)*i.texPos*0.005 + 0.005;
